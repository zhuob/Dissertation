...
%	connections between CAMERA and our method. 
%%	First, compared to the model (\ref{eq:overview}), CAMERA does not have the random effect term 
%%$\psi_i$, which means CAMERA .
%	%We note that, under our framework, CAMERA effectively assumes that $\rho_2=\rho_3=0$ (i.e, 
%	%between-gene correlations are present only among genes in the test set), and that 
%	%$\sigma_1^2=\sigma_2^2=0$ 
%	%(i.e, the $\psi_i$ is absorbed into the fixed terms $\beta_0$ and $\beta_1$).
...
%	To explore how violation of them affects the performance of CAMERA, note that when 

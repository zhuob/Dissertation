...
\usepackage[nottoc,notlot,notlof]{tocbibind}  % show bibliography in TOC
\usepackage{lscape}  % Vertical Display table
\usepackage{chngcntr}  % Count Figure within chapter/section
\usepackage{tablefootnote}  % for table at appendix
...
\newcommand{\thepapertobefinished}{as we have seen in Chapter \ref{chap2}}
...
%-------------------------------------------------------
    ...
    \def\CommencementMonth{June 2016}
     ...
     %%Thesis Information
    \def\ThesisTitle{Higher-level Analysis of RNA-Seq Experiment: Multiple Data Sets and 
    Multiple Genes}
    \def\DatePresented{June 22, 2016}
    \def\keywords{RNA-Seq, stably expressed genes, correlation, gene-set test} %Up to five keywords
...
\begin{document}  
  ...
  \ThesisTitlePage
  \ThesisApprovalPage
  \ThesisAcknowledgmentPage{\input{acknowledge.tex}}
  %End Thesis Pre-text

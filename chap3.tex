...
%	among gene expression levels.
	...
	We have mentioned in the Introduction part that the test statistics correlations among genes may
	...
	may
	not perform well. \underline{(a heads-up on how the 2 groups are different: if non-GO-term 
	genes have DE)} 
	To illustrate this point, we perform two groups of simulations ($A_1$ and $A_2$) for each of 
	...
	background set. That is, genes in the background set are allowed to be DE in group $A_2$ but 
	not in group $A_1$.
...
%		structure a (uncorrelated genes).}
	...
	\begin{table}[!ht]
		\centering
		\caption{Recalibrated power for different methods. The powers are 
		summarized
